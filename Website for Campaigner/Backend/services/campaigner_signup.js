...
const campaignersignup = async (signupDetails) => {
  ...
  const result = await sqlConn.connection.query(`SELECT * FROM campaigner WHERE email LIKE '${signupDetails.email}'`);
  ...
  const insert = await sqlConn.connection.query(`INSERT INTO campaigner (name, email, password, phone, cnic, officeaddress) VALUES ('${signupDetails.name}', '${signupDetails.email}', '${password}', '${signupDetails.phone}', '${signupDetails.cnic}', '${signupDetails.officeAddress}')`);
  console.log(insert)
